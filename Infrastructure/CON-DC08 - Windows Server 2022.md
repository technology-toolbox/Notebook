# CON-DC08 - Windows Server 2022 Domain Controller
...
- On the **Computer Details** step:
  - In the **Computer name** box, type **CON-DC08**.
...
**CON-DC06** - Run as domain administrator
...
```PowerShell
Add-ADGroupMember -Identity "Windows Update - Slot 21" -Members "CON-DC08$"
```
...
```PowerShell
Get-ADComputer CON-DC06 | Remove-ADObject -Recursive -Confirm:$false
```
...
```PowerShell
Remove-SCVirtualMachine CON-DC06
```
...
(Get-ClusterGroup `
    -Cluster TT-HV06-FC `
    -Name 'SCVMM CON-DC07 Resources' `
    ).AntiAffinityClassNames = $antiAffinityClassNames

(Get-ClusterGroup `
    -Cluster TT-HV06-FC `
    -Name 'SCVMM CON-DC08 Resources' `
    ).AntiAffinityClassNames = $antiAffinityClassNames
...
Set-WBSchedule -Policy $policy 15:00
...
$trigger = New-ScheduledTaskTrigger -Daily -At '2:55 PM'
